...
function(doc) {
  ...
  if (name) {
    for (var x=1; x<=name.length; x++) {
      if ((name.substr(0, x).trim() === '') || (name.substr(0, x).trim() === '_')) continue;
      if (name[x] !== ' ') {
        if (seen.indexOf(name.substr(0, x).toLowerCase().trim()) === -1) {
          emit(name.substr(0, x).toLowerCase().trim(), { guid: guid, name: name });
          seen.push(name.substr(0, x).toLowerCase().trim());
        }
      }
      if (name[x-1] !== ' ') {
        for (var y=x; y<=name.length; y++) {
          if (seen.indexOf(name.substring(x-1, y).toLowerCase().trim()) === -1) {
            emit(name.substring(x-1, y).toLowerCase().trim(), { guid: guid, name: name });
            seen.push(name.substring(x-1, y).toLowerCase().trim());
          }
        }
      }
    }
...
const couchdb = 'http://gctools-couchdb.gctools-recommendation:5984/profiles/_design/finder/_view';
...
const profile_service
  = 'http://recommendation-storage.gctoolsv2/profile';
...
const getUserData = (user, selections) => {
  /**
   * Retreive user data from sources such as GCconnex, GCprofile, etc
   */
  const { missing_selections } = isDataAvailable(user, selections);
  ...
  const gcprofile_fields = ['phraseCloud', 'gcconnex_username'];
  ...
  if (missing_selections.some(v => gcconnex_fields.includes(v))) {
    promises.push(new Promise((resolve, reject) => {
      const uri = encodeURI(`${gcconnex}/user/${guid}`);
      request({
        uri,
        headers: {
    ...
    promises.push(new Promise((resolve, reject) => {
      ...
      request({
        ...
        if (error) {
          ...
          if (!profile) return {};
          const obj = {};
          if (missing_selections.includes('phraseCloud') && profile.pc) {
            const { pc } = profile;
          ...
          if (missing_selections.includes('gcconnex_username')) {
            obj.gcconnex_username = profile.username;
          };
          resolve(obj);
        } else {
          resolve({});
...
export const personResolver = (root, args, context, info) => {
      ...
      Promise.all(queries).then((data) => {
        ...
        if (args.limit === 0) {
          ...
          for (let x = 0; x < data[0].length; x += 1) {
            counter += 1;
            res.push(getUserData(data[0][x], selections));
          ...
          for (let x = 0; x < data.length; x += 1) {
            for (let y = 0; y < data[x].length; y += 1) {
              ...
              if (add) {
                counter += 1;
                res.push(getUserData(data[x][y], selections));
                if (counter === args.limit) break;
              }
            }
            if (counter === args.limit) break;
  ...
  return new Promise((resolve, reject) => {
    if (root.gcconnex_guid != '-1') {
      getUserData({id: root.gcconnex_guid}, selections)
        .then(data => {
          resolve(data)
        })
        .catch(err => reject(err));
