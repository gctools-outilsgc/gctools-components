...
DOCKER_PACKAGE="gctools-outilsgc/graphql-server"
...
then
  ...
  CMD=$(echo sed -i "'s/localhost:5000\/gctools-outilsgc\/reference-implementation:$CURRENT/localhost:5000\/gctools-outilsgc\/reference-implementation:$PREVIOUS/g'" $DIR/0_deployment.yaml)
  eval $CMD
