...
then
  ...
  CMD=$(echo sed -i "'s/localhost:5000\/gctools-outilsgc\/recommendation-store:$CURRENT/localhost:5000\/gctools-outilsgc\/recommendation-store:$PREVIOUS/g'" $DIR/0_deployment.yaml)
  eval $CMD
